jQuery(document).ready(function($){
		...
		if(typeof($(this).parent().attr("id"))!="undefined")
		{
			...
			$.ajax({
					...
					success: function(json){
						...
						var indexEnd = json.indexOf("class_hour_end")-16;
	...
	if($(".color").length)
	{
		$(".color").ColorPicker({
			...
			$(this).ColorPickerSetColor(this.value);
			$(this).prev(".color_preview").css("background-color", (this.value!="" ? "#" + $(".colorpicker:visible .colorpicker_hex input").val() : "transparent"));
		});
	}
