...
class WP_REST_Users_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/mobile'  . '/(?P<mobile>[\d]+)', array(
			array(
				'methods'         => WP_REST_Server::READABLE,
				'callback'        => array( $this, 'get_item_by_mobile' ),
				//'permission_callback' => array( $this, 'get_item_permissions_check' ),
				'args'            => array(
					'context'          => $this->get_context_param( array( 'default' => 'view' ) ),
				),
   ...
   /**
	 ...
	 * @return WP_Error|WP_REST_Response
	 */
	public function get_item_by_mobile ( $request ) {
		$mobile = $request['mobile'];
		$user = get_userdata( $mobile );
        $meta_key = 'phone';
        $user = reset(
        get_users(
          array(
		...
		$schema = array(
			...
			'properties' => array(
				...
				'password'        => array(
					...
					'type'        => 'string',
					'context'     => array('edit'), // Password is never displayed
					'required'    => true,
				),
				'capabilities'    => array(
